...
If you use the `--develop` attribute, the program create a docker volume to override the "original" `opt/wf-docker-workflow` directory with the cloned and edited `[project dir]/packages/wf-docker-workflow/src/opt/wf-docker-workflow` directory. Now you can test and check the working with the new code(s). **It is important**: the program will use your `~/.wf-docker-workflow/config/*` files! If you want to play with configs you have to test with your "host" file, and then you have to copy the changes to "here".
...
> You can change the behavior in the **packages/wf-docker-workflow/src/opt/wf-docker-workflow/lib/_functions.sh** file
