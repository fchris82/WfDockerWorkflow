...
# `~/.webtown-workflow/bin/zsh_autocomplete.sh` file.
...
_wf() {
    ...
    case $state in
        command)
            _arguments '1: :(-h --help --version -ps --docker-ps --composer-install --reload --clean-cache --enter --run --sf-run --extensions --config-dump reconfigure --update --rebuild)'
